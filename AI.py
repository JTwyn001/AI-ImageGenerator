...
def generate():
    ...
    for url in image_urls:
        ...
        canvas.image_references.append(photo_image)

    def update_image(index=0):
        current_image = canvas.image_references[index]
        canvas.image = current_image
        canvas.create_image(0, 0, anchor="nw", image=current_image)
        index = (index + 1) % len(canvas.image_references)
...
style_dropdown = ctk.CTkComboBox(input_frame, values=["Realistic", "Cartoon", "3D Illustration", "Art"])
